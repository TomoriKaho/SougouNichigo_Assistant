...
function generationPrompt({ range, user, grammar, vocabulary, textSamples, directionMode }) {
  ...
  const vocabularyLines = vocabulary.slice(0, 12).map((item, index) => (
...
题目要有文学性但清楚，不要复述现代作品。能力标签是上限，不要故意顶格出难题。
`.trim()
    },
    {
      role: 'user',
...
2. 题目应是文学性但清楚的短段落，避免纯口语闲聊，长度 80-140 字符左右。
3. 这一题必须尽量自然覆盖全部目标文法；不要机械堆叠。
4. 如果是汉译日题，必须列出“本题目标文法”，只写文法和接续，不写中文意思。
5. 如果使用超出所选范围或难度上限的词汇/表达，在 advanced_notes 标注词、读音/说明、中文释义、为什么需要注明。
6. 尽量优先使用已录入词汇；不要为了炫技堆叠高难表达。
...
【JSON Schema】
{
  ...
  "issues": [
    {"item_id":"zh_jp","severity":"serious","category":"语法","quote":"用户译文中的问题片段","explanation":"问题说明","suggestion":"修改建议"},
    {"item_id":"jp_zh","severity":"minor","category":"自然度","quote":"...","explanation":"...","suggestion":"..."}
  ],
  "grammar_focus": [
...
function chatPrompt({ practice, messages, question }) {
...
function normalizeGeneratedExercise(parsed, grammar, directionMode = 'jp_to_zh') {
...
async function generatePractice({ userId, rangeKey, directionMode }) {
  ...
  const vocabularySample = pickItems(vocabulary, Math.min(28, vocabulary.length))
  ...
  let exercise
  try {
    exercise = normalizeGeneratedExercise(parsed, grammar, normalizedDirectionMode)
  } catch (error) {
  ...
  return TranslationPractice.create({
    ...
    status: 'draft',
    grammar,
    vocabulary: vocabularySample.slice(0, 24),
    exercise
  })
}
...
async function submitPractice({ userId, practiceId, answers }) {
  ...
  const review = normalizeReview(extractJson(raw))
...
async function askPractice({ userId, practiceId, content }) {
  ...
  const answer = await completeChat({
    messages: chatPrompt({ practice, messages, question }),
    enableSearch: false,
    forcedSearch: false,
    maxTokens: 1200
  })
