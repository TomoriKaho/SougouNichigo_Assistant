...
function backupFile(filePath) {
  ...
  const backupPath = `${filePath}.${stamp}.before-import-grammar1.bak`
...
function main() {
  ...
  const source = getGrammarSources(dataDirectory).find((item) => item.key === 'first')
  if (!source) throw new Error('未配置第一册文法数据源')
  ...
  try {
    ...
    const summary = db.prepare(`
      SELECT
        ...
        SUM(CASE WHEN content_markdown IS NULL OR content_markdown = '' THEN 1 ELSE 0 END) AS entries_without_markdown,
        SUM(CASE WHEN content_markdown LIKE '%| ---%' THEN 1 ELSE 0 END) AS entries_with_tables
