...
function backupFile(filePath) {
  ...
  const backupPath = `${filePath}.${stamp}.before-import-grammar2.bak`
...
function main() {
  ...
  const source = getGrammarSources(dataDirectory).find((item) => item.key === 'second')
  if (!source) throw new Error('未配置第二册文法数据源')
